...
<script>
  export default {
    ${getHtml('showName', 'name')}${getHtml('showProps', 'props')}${getHtml('showData', 'data')}${getHtml(
			'showComponents',
			'components'
		)}${getHtml('showBeforeCreate', 'beforeCreate')}${getHtml('showCreated', 'created')}${getHtml(
			'showBeforeUpdate',
			'beforeUpdate'
		)}${getHtml('showUpdated', 'updated')}${getHtml('showBeforeDestroy', 'beforeDestroy')}${getHtml(
			'showDestroyed',
			'destroyed'
		)}${getHtml('showMethods', 'methods')}${getHtml('showWatch', 'watch')}${getHtml('showComputed', 'computed')}
  }
</script>
...
function activate(context) {
	let disposable = vscode.commands.registerCommand('extension.new-vue-file', function(obj) {
		vscode.window
			.showInputBox({
